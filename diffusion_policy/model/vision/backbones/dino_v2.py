...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        if load_from is not None:
            ...
            if len(missing_keys) > 0:
                logger.warning(f"[{self.__class__.__name__}] missing keys: {missing_keys}")
            if len(unexpected_keys) > 0:
                logger.warning(f"[{self.__class__.__name__}] unexpected keys: {unexpected_keys}")
    ...
    def forward(self, *args, **kwargs):
        ret = self.forward_features(*args, **kwargs)[-1]
        return ret
