...
class TrainManipForceWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        logger.info(f'train dataset: {len(dataset)}, train dataloader: {len(train_dataloader)}')
        logger.info(f'val dataset: {len(val_dataset)}, val dataloader: {len(val_dataloader)}')
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if hasattr(model_unwrapped, '_ft_loss'):
                            step_log['ft_loss'] = model_unwrapped._ft_loss
                        ...
                        if (cfg.training.max_train_steps is not None) \
                            ...
                            break
                logger.info(f"Training epoch {self.epoch} finished")
                ...
                # run validation
                if ((self.epoch + 1) % cfg.training.val_every) == 0 and len(val_dataloader) > 0 and accelerator.is_main_process:
                    with torch.no_grad():
                        val_losses = list()
                    ...
                    logger.info(f"Validation epoch {self.epoch} finished")
                
                def log_action_mse(step_log, category, pred_dict, gt_action):
                    pred_action = pred_dict['action_pred']
                    ...
                    # Add delta_p norm logging
                    if 'delta_p' in pred_dict:
                        step_log[f'{category}_delta_p_norm'] = torch.norm(pred_dict['delta_p'], dim=-1).mean().item()
                    if 'F_pred' in pred_dict:
                        step_log[f'{category}_F_pred'] = pred_dict['F_pred'].mean().item()

                # run diffusion sampling on a training batch
                if ((self.epoch + 1) % cfg.training.sample_every) == 0 and accelerator.is_main_process:
                    with torch.no_grad():
                        batch = dict_apply(train_sampling_batch, lambda x: x.to(device, non_blocking=True))
                        gt_action = batch['action']
                        pred_dict = policy.predict_action(batch['obs'])
                        log_action_mse(step_log, 'train', pred_dict, gt_action)
                        ...
                        if len(val_dataloader) > 0:
                            ...
                            gt_action = batch['action']
                            pred_dict = policy.predict_action(batch['obs'])
                            log_action_mse(step_log, 'val', pred_dict, gt_action)
...
def main(cfg):
    workspace = TrainManipForceWorkspace(cfg)
    workspace.run()
