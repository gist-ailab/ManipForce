...
# rs_capture의 기본 intrinsics를 재사용
def get_azure_kinect_intrinsics():
    cap = AzureImageCapture()
    return cap.get_default_intrinsics()
...
def process_images_from_directory(image_dir, output_json_path, visualize=False):
    ...
    # Azure Kinect 실시간 연결 제거 - 하드코딩된 파라미터 사용
    K_azure, D_azure = get_azure_kinect_intrinsics()
        ...
        if aruco_pose is not None:
            ...
            if visualize:
                ...
                if gripper_vis is not None:
                    # 작은 크기로 리사이징하여 성능 향상
                    vis_pose_small = cv2.resize(image_with_axes, (320, 240))
                    vis_gripper_small = cv2.resize(gripper_vis, (320, 240))
